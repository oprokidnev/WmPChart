         ...
         if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
          {
           ...
           if ( $Rounded || $BorderR != -1) { $XSpace = 3; } else { $XSpace = 0; }
                     ...
                     if ( $GradientMode == GRADIENT_SIMPLE )
                      {
                       ...
                       $this->drawGradientArea($X+$XOffset+$XSpace,$Y1,$X+$XOffset+$XSize-$XSpace,$Y2,DIRECTION_HORIZONTAL,$GradienColor);
                      }
               ...
               if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
                {
                 ...
                 if ( $DisplayPos == LABEL_POS_INSIDE && abs($TxtHeight) < abs($BarHeight) )
                  ...
                  {
                   if ( $Serie["Data"][$Key] >= 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
                   $this->drawText($X+$XOffset+$XSize/2+$DisplaySize/3,$Y2-$Offset-$XSize/2,$this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align,"FontSize"=>$DisplaySize,'Angle'=>90));
                  }
               ...
               if ( $DisplayValues && $Serie["Data"][$Key] != VOID ) 
                {
                 ...
                 if ( $DisplayPos == LABEL_POS_INSIDE && abs($TxtWidth) < abs($BarWidth) )
                 ...
                 else
                  {
                   ...
                   $this->drawText($X2+$Offset,$Y+$YOffset+$YSize/2,$Caption,array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align,"FontSize"=>$DisplaySize,'Angle'=>90));
                  }
